...
| G5 | M | `config/schema.json` did not describe the accepted aliases / value forms | **Fixed** | — |
| G6 | M | Rule **R06 chameleon** is a stub: `chameleon_flag` is hard-coded False, so the rule can never fire | Open; marked DORMANT in `config/rules.yaml` | Needs the revoked-carrier address/phone index from the census file (~45 min) |
...
| G8 | M | **SERFF rate filings (S14) not pulled** — the best public benchmark for relativity structure and new-venture factors | Open, and **not on the critical path**: the brief asks for a rater, rules and a book check, none of which SERFF gates. The price *level* is already anchored to ten observed market points (S16); SERFF would cross-check the relativity *structure*. It is DEFEND §4 roadmap item 3, 1–2 h with a state DOI portal | Where it does bite is A3: 1.65 is selected inside a 1.3–2.0 band that no filing was read to confirm. RATIONALE A3 now says so, and validates the factor at the price via benchmark P5 instead |
...
| G10 | M | **L&I (S6) not automated** — D23 (insurance cancelled in 24 months) reads the submission's self-declared field | Open; roadmap item 2 | Scrape L&I for insurer, policy dates and cancellation reason, cache per DOT, and read it in `rater/enrich.py`; say so plainly in the walkthrough |
| G11 | M | **Physical damage** (bonus) is a design, not code | Open | DEFEND §5.2 has the design: `config/pd_rates.yaml` + `rater/physdam.py`, stated value from vPIC through a depreciation table, comp and collision as a share of value, exposed as a `--with-pd` flag |
...
| G18 | L | Remaining VERIFY markers: A2 claims-per-crash (only verifiable in force), A8 trend figures vs the ATRI PDFs, A12 OOS averages (G9), S7 SMS methodology change | Open by nature — A2 needs a bound book, and the rest are confirmations of figures already sourced through trade press | Each is tagged inline in `config/rates.yaml` and `docs/RATIONALE.md` §5 with the range considered, so the exposure is visible at the point of use rather than in a list |
