...
- **Carrier's own experience** enters through Bühlmann credibility, Z = n/(n+25) with n = units × 2 years. A one-truck carrier with one crash gets Z ≈ 0.07 and about a 15% surcharge; the decline rule for own experience (D20) can only fire for larger fleets with repeated crashes. Single crashes on tiny fleets are noise, and the model says so.
...
From the 45-carrier census draw (`data/derived/book_check_real.csv`): 17 priced, 17 referred, 11 declined.

- **Priced:** small, mostly newer general-freight carriers in mid-venue states, $8k–$16k per unit; clean established owner-operators at the $8k floor (bottom of the published $7.5k–$11k range); median written policy ≈ $13k.
...
5. `python -m rater.book samples/submissions their_folder` — their cases alongside ours.
6. `config/rates.yaml` open on the loss-cost block; `data/derived/tornado.csv`; `docs/MARKET_BENCHMARK.md` after table.
