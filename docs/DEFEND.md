...
**Volunteer the sampling caveat before they find it.** The 24% decline rate is partly an artifact of how the sample was drawn. The census `authorized_for_hire` flag is self-declared on the MCS-150 and goes stale, so a random census draw over-samples dormant shells no broker would ever submit. Of the 45 drawn carriers, only **23 (51%)** have active for-hire authority *and* BI/PD insurance on file. `analysis/build_operating_set.py` re-draws the same segment with that operating test applied, and on those 38 carriers the rater declines **1 (2.6%)** and prices 37 at a median $9,943 per unit (`book_check_operating.csv`). Both numbers are true and they answer different questions: 24% is what a census draw declines, 2.6% is what a submission flow declines, and the gap is dormancy, not appetite. The honest planning number for a real quote flow is nearer the second.
...
`docs/MARKET_BENCHMARK.md`: ten 2025–26 price points (Progressive published averages, Overdrive, FreightWaves, CCJ / ATRI, FleetOwner, Geotab) mirrored as synthetic submissions. Current figures, after the three calibration changes, the re-basing and the 2026-09-10 credibility and floor corrections: **median gap +2.8%**, mean +5.4%, 6 of 10 within ±20% of the liability-only estimate, **8 of 10** within ±20% of the published figure (median gap to published −5.1%). Before the credibility correction the median gap was −1.2%; removing the off-balance leak raised every non-floor profile by 4–6%, which is the whole of the move. The two largest overshoots (P1 +41%, P2 +29%) depend on my 70% liability share of Progressive's bundled premium — against the published figures they are −1% and −9% — so they were not tuned further. The honest miss is P6 at −29%: a five-year, late-model, Indiana, intermediate-radius owner-operator at the $8k floor against a $7.5k–$15k published range.
...
- *"Your own GLM says solo carriers crash 18–27% more per unit. Your manual gives them a 5% discount. 80% of your book is solo. Explain."* **The sharpest question in the pack, and the answer is the market benchmark.** The measurement is real: 1.176, SE 0.013, operating cut 1.224, raw 3y+ rates 0.0462 vs 0.0363. I did not apply it, for three reasons in increasing order of force. First, it is a *frequency* contrast measured on census exposure, and 2–5 unit carriers overstate power units on the MCS-150, which dilutes their per-unit rate and inflates the solo relativity — which is why FREQUENCY_PROPOSAL §3 calls 1.18 an upper bound and 1.10–1.18 the honest range. Second, only the *product* of crash rate × claims-per-crash × relativities is identified, and that product is pinned to the ten observed market prices; moving one factor up 24% without moving another down breaks the anchor. Third and decisively, **I tested it**: at 1.15 the one-unit benchmark profiles go to +71% (P1), +50% (P3) and +44% (P7) against the observed liability-only estimates, from +41 / +24 / +19 today. The market charges solo carriers roughly what I charge them, and I am not going to price the most common risk in the segment out of the band on a frequency contrast I know is biased upward. If the contrast *is* real, the reconciliation is that solo severity is lower — owner-driver, no dispatch pressure to run tired — and offsets it. That is a bound-book question, and it is item 5 on the roadmap. RATIONALE A16.
